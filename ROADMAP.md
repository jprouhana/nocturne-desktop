...
- [x] **Like / unlike**: a heart on each row + in the player bar; `POST /api/like`
...
- [x] **Queue / up-next panel**: a toggle showing `GET /api/queue` with the
      current track highlighted; click to jump; drag-free for now.
- [x] **Theme switching**: expose the 7 ytm.py themes; `c` cycles; the GUI
      accent (`--accent`, gradients, lyrics) follows the active theme. Persist
      choice to a small `~/.config/nocturne-desktop/state.json`.
- [x] **Add to queue / add to playlist**: right-click or hover button on a row
      → enqueue, or pick a playlist (`yt` library write ops already in ytm.py).
- [x] **Repeat / shuffle**: transport toggles; backend honors them on EOF.
- [x] **Now-playing full view**: a big-art "now playing" screen (art, title,
      progress, lyrics side-by-side) as an alternate to the list.
- [x] **Window + volume persistence**: remember window size/pos and last volume
      in the state.json.
- [x] **Polish pass**: loading skeletons, empty states, error toasts when the
...
- like/unlike: ♥ in player bar + Y key, POST /api/like (yt.rate_song), liked reflected from get_watch_playlist.
- queue panel: q toggle, GET /api/queue, double-click /api/queue_jump.
- theme switching: GET /api/themes (7 ytm.py palettes), c cycles, applies CSS vars, persisted to localStorage.
- add to queue/playlist: hover ＋ enqueues; POST /api/add_to_playlist (yt.add_playlist_items).
- repeat/shuffle: transport buttons + r/s keys; backend honors repeat (off/all/one) on EOF, shuffle reorders queue.
